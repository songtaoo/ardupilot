...
public:
    ...
    enum class Number : uint8_t {
        ...
        BFACRO =       29,  // Betaflight-style body-frame angular rate control with manual throttle
...
class ModeBFAcro : public Mode {

public:
    using Mode::Mode;

    Number mode_number() const override { return Number::BFACRO; }

    enum class BFAcroOptions {
        AIR_MODE = 1 << 0,
    };
...
protected:
    const char *name() const override { return "BFACRO"; }
    const char *name4() const override { return "BFAC"; }

    float throttle_hover() const override;

private:
    void get_pilot_desired_rates(float &roll_out, float &pitch_out, float &yaw_out) const;
    float apply_betaflight_expo(float input, float expo) const;
